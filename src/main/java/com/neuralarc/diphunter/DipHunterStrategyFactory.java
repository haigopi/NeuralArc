...
public final class DipHunterStrategyFactory {
    public Strategy toStrategy(DipHunterRecommendation recommendation, String workspaceId, boolean executeRequested, int pollingSeconds) {
        ...
        Strategy strategy = Strategy.fromConfig(
                ...
                "DIP_HUNTER: " + recommendation.symbol() + " " + recommendation.mode().name(),
                config,
                recommendation.mode()
        );
        ...
        strategy.setLatestOrderStatus(executeRequested ? "DIP_HUNTER_MONITORING" : "DIP_HUNTER_RECOMMENDED");
        strategy.setLatestAlpacaOrderId("");
        strategy.setLastEvent("Dip Hunter " + (executeRequested ? "monitoring" : "recommendation")
